...
%right '.' arr
%right if then else let in await
%left ' '
%right '=' 
-- this might lead to problems - might need to have separate symbols for bullet type and bullet operator
%right '*'      
...
TupleAccessor : Expr '.' nzdigit                 { % checkTupleAccessorPair $1 $3 }
              | Expr '.' nzdigit '(' nzdigit ')' { % checkTupleAccessor $1 $3 $5 }
...
-- Checking tuple index vs tuple size
checkTupleAccessor :: RawExpr -> Int -> Int -> Either Error RawExpr
checkTupleAccessor tupExpr index size = 
    if index <= size
    then return $ RTupAcc tupExpr index size
    else Left $ ParseFailure $ "Parsing failure: accessor ." ++ show index ++ "(" ++ show size ++ ") is not possible."
                 
checkTupleAccessorPair :: RawExpr -> Int -> Either Error RawExpr
checkTupleAccessorPair tupExpr index =
    if index <= 2
    then return $ RTupAcc tupExpr index 2
    else Left $ ParseFailure $ "Parsing failure: accessor ." ++ show index ++ " is not possible given inferred tuple size of 2."
