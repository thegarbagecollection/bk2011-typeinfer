...
toExpr :: RawExpr -> Either Error Expr
toExpr r = fmap fst $ runStateT (toExpr' (S.fromList builtInAbs) r) (VC (M.empty, M.empty) M.empty)

toExprInit :: S.Set String -> RawExpr -> Either Error Expr
toExprInit initialAbs r = fmap fst $ runStateT (toExpr' initialAbs r) (VC (M.empty, M.empty) M.empty)
...
toExpr' (RLit l) = do
    return $ Lit l
-}
toExpr' initialAbs (RLit l) = pure $ f l
    where f (RBoolLit b) = Lit $ BoolLit b
          f RNatLitZero = Lit $ NatLitZero
...
toExpr' initialAbs (RTupAcc re digit) = do
    e <- toExpr' initialAbs re
    return $ ETupAcc e digit
