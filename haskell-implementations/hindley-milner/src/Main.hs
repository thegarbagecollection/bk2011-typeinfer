...
-- to add a constraint
type Gamma = M.Map String GammaElem
...
runRIPL gammaInit replAction n = do
    putStrLn ""
    l <- getLine
    if l == "Q" || l == "q"
    then return ()
       ...
       case binding input of
            Nothing -> putStrLn "Binding must be in format @name=body" >> runRIPL gammaInit' replAction n'
            
            Just (Left input) -> do
                _ <- replAction (input, gammaInit', n)
                runRIPL gammaInit' replAction n'
                
            Just (Right (name, bodyInput)) -> do
                mt <- replAction (bodyInput, gammaInit', n)
                case mt of
                    Nothing -> runRIPL gammaInit' replAction n
                    Just t -> runRIPL (bindTypeInGamma name t gammaInit') replAction n'
    
    
-- standard RIPL - don't save files
ripl :: IO ()
ripl = do
    putStrLn "Hindley-Milner inference: standard RIPL, no file output; q or Q to quit"
    runRIPL builtInAbsType basicRIPLAction 1
...
writerRIPL filePrefix = do
    putStrLn $ "Hindley-Milner inference: file-writer RIPL, result output to directory /" ++ filePrefix ++ ", ; q or Q to quit"
    ensureDirectories
...
--                                                  name -> inferred time
-- type ReplInput = (InputString, GammaWithTimes)
-- inferTypeFromGammaAndTimeConstraints :: Expr -> M.Map String GammaElem -> TimeConstraints -> Either Error (Type, TimeStep)
basicRIPLAction :: RIPLInput -> IO (Maybe Type)
basicRIPLAction (input, gammaInit, iter) = do
...
writerRIPLAction filePrefix (input, gammaInit, currentIteration) = do
    case buildExprUsingExplicitBuiltInNames (S.fromList . M.keys $ gammaInit) input of
        Left err -> (putStrLn . show $ err) >> return Nothing
        Right expr -> do
            let eitherConstraints = constraintsTemp2 gammaInit $ Right expr
                inferred = inferTypeFromGamma expr gammaInit
            case eitherConstraints of
                Left _ -> return ()
                Right (_, typeConstraints) -> do
...
freshGammaInit gammaInit = freshenTypeVars gammaInit
    where               
        -- all elements of gamma are of no variables bound, they're all empty type schemes
          ...
          foldFreshen :: String -> GammaElem -> (Gamma, Integer) -> (Gamma, Integer)
          foldFreshen name gammaElem (m, n) = 
              let (freshened, nextTypeVar) = doFreshen (extractTypeFromGammaElem gammaElem) n
...
extractTypeFromGammaElem (GammaVarType t) = t
extractTypeFromGammaElem (GammaTypeScheme _ t) = t

    
    
bindTypeInGamma :: String -> Type -> M.Map String GammaElem -> M.Map String GammaElem
bindTypeInGamma name t = M.insert name (emptyTypeScheme t) -- doesn't matter what type or time names are here, we'll refresh them every iteration
